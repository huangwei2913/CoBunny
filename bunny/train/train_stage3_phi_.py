...
def train():
    ...
    # =========================================================
    # ➕ [新增] LoRA 初始化逻辑
    # 必须放在硬核补丁之前，否则 LoRA 参数会被后面的补丁锁死！
    # =========================================================
    if training_args.lora_enable:
        from peft import LoraConfig, get_peft_model
        rank0_print("🎨 [LoRA] 正在为 Phi-1.5 挂载 LoRA 适配器...")
        lora_config = LoraConfig(
            r=training_args.lora_r,
            lora_alpha=training_args.lora_alpha,
            target_modules=["q_proj", "v_proj", "k_proj", "dense", "fc1", "fc2"], # 适配 Phi 的注意力层和 MLP 层
            lora_dropout=0.05,
            bias="none",
            task_type="CAUSAL_LM",
        )
        model = get_peft_model(model, lora_config)
        # 挂载后，打印一下 LoRA 的参数量，心里有个底
        if training_args.local_rank == 0 or training_args.local_rank == -1:
            model.print_trainable_parameters()


    # =========================================================
    rank0_print("🔒 [Hardcode Strategy] 正在强制接管梯度控制权：锁死 LLM 基座，全力放开视觉、投影与 LoRA...")

    frozen_llm_layers = 0
    unfrozen_vision_layers = 0
    unfrozen_projector_layers = 0
    unfrozen_lora_layers = 0 # 新增统计

    for name, param in model.named_parameters():
        # 1. 识别并放开投影层
        if "mm_projector" in name:
            param.requires_grad_(True)
            unfrozen_projector_layers += 1
            
        # 2. 识别并放开视觉塔全家桶
        elif "vision_tower" in name:
            param.requires_grad_(True)
            unfrozen_vision_layers += 1
            
        # 3. [新增] 识别并放开 LoRA 权重
        elif "lora_" in name:
            param.requires_grad_(True)
            unfrozen_lora_layers += 1
            
        # 4. 剩下的全都是 LLM 基座原生参数，统统锁死！
        else:
            param.requires_grad_(False)
            frozen_llm_layers += 1

    rank0_print(f"📊 [硬核补丁执行结果]")
    rank0_print(f"   ❄️ 已冻结 LLM 基座层数: {frozen_llm_layers} 层 (原生智商已锁定)")
    rank0_print(f"   🔥 已解冻 视觉感知层数: {unfrozen_vision_layers} 层 (眼睛已放开)")
    rank0_print(f"   🔥 已解冻 投影对齐层数: {unfrozen_projector_layers} 层 (翻译官已放开)")
    rank0_print(f"   🔥 已解冻 LoRA 适配器层数: {unfrozen_lora_layers} 层 (微调外挂已启动)")
