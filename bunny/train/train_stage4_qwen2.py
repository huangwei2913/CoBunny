...
def train():
    ...
    model.config.training_stage = "finetune"  # 确保传给 config
...
# =========================================================
# 🧬 [V365 协议 - 终极全量解冻：刺穿套娃与 GPU 屏障]
# =========================================================
    rank0_print("\n" + "💉" * 20)
    rank0_print("🚀 正在执行 Stage 4 强制全量解冻：哪怕在 GPU 深处也要把它揪出来...")
    rank0_print("💉" * 20)

    # 1. 确保模式正确：全部切回 train
    model.train()
    # 强制让那个嵌套的 vision_tower 也醒过来
    if hasattr(model, 'get_vision_tower'):
        model.get_vision_tower().train()

    # 2. 全量锁定（先清空状态）
    model.requires_grad_(False)

    # 3. 核心标签（直接匹配你 TXT 里的层名关键词，无视路径）
    # 这一组涵盖了你所有的 Sampler, CrossAttn, Gate 和 Projector
    v_core_tags = [
        "mlp_layers", "dino_projector", "siglip_projector", 
        "saliency_sampler", "cross_attn", "gate_mlps", "mm_projector"
    ]
    # LLM 关键层
    l_core_tags = ["embed_tokens", "lm_head", "layers.25", "layers.26", "layers.27"]

    unlocked_params = []
    # 4. 强制解冻逻辑
    for name, param in model.named_parameters():
        # 只要名字里包含上述任何一个词，不管它在哪个显卡上，强行开启梯度
        if any(tag in name for tag in v_core_tags) or any(tag in name for tag in l_core_tags):
            param.requires_grad = True
            unlocked_params.append(name)

    # 5. 专家级审计（解决统计不准的问题）
    if training_args.local_rank <= 0:
        # 统计所有 requires_grad=True 的参数，不管它在哪个 Device
        total_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
        
        rank0_print("\n" + "="*60)
        rank0_print(f"📊 [V365 终极审计]：全量解冻参数总数: {total_trainable / 1e6:.2f} M")
        rank0_print(f"📍 激活 Tensor 总数: {len(unlocked_params)}")
        rank0_print("-" * 60)
        
        # 核查关键点
        has_cross = any("cross_attn" in n for n in unlocked_params)
        has_sampler = any("saliency_sampler" in n for n in unlocked_params)
        
        rank0_print(f"🔍 交叉注意力 (CrossAttn): {'✅ 已激活' if has_cross else '❌ 没找到'}")
        rank0_print(f"🔍 采样器 (Sampler):     {'✅ 已激活' if has_sampler else '❌ 没找到'}")
        
        if total_trainable / 1e6 < 350:
            rank0_print("\n🛑 警告：参数量还是只有 200多M？")
            rank0_print("💡 黄老师，如果还是这数，说明那些 Linear 层的 Weight 真的没被 named_parameters 吐出来。")
        else:
            rank0_print("\n💎 [SUCCESS] 视觉心脏全线起搏！")
        rank0_print("="*60 + "\n")

    # 6. 开启显存优化，否则 8 块 T4 扛不住全量解冻后的梯度
